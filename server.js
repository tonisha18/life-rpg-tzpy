...
try {
  ...
  try {
    ...
    console.error('Failed to load "next" module. Please run "npm run install:all" or "npm install".');
    process.exit(1);
  }
}
...
app.prepare().then(() => {
  ...
  // Handle termination signals for cloud deployment (containers, PM2, systemd, PaaS)
  const shutdown = () => {
